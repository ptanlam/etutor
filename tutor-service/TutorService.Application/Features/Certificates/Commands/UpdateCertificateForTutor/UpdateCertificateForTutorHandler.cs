...
{
    public class
        UpdateCertificateForTutorHandler : IRequestHandler<UpdateCertificateForTutor, (bool found,
            List<ValidationFailure> errors, CertificateVm certificate)>
    {
        private readonly ITutorsRepository _tutorsRepository;
        private readonly IValidator<UpdateCertificateForTutor> _validator;
        private readonly IMapper _mapper;

        public UpdateCertificateForTutorHandler(ITutorsRepository tutorsRepository,
            IValidator<UpdateCertificateForTutor> validator, IMapper mapper)
        {
        ...
        public async Task<(bool found, List<ValidationFailure> errors, CertificateVm certificate)> Handle(
            UpdateCertificateForTutor request, CancellationToken cancellationToken)
        {
            ...
            certificate.UpdateName(request.Name);
            certificate.UpdatePlaceOfIssue(request.PlaceOfIssue);
